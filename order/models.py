...
class Order(models.Model):
    ...
    freight = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00, verbose_name="运费"
    )
...
class OrderItem(models.Model):
    ...
    image = models.ImageField(
        upload_to="product_image", verbose_name="商品图片", blank=True, null=True
    )
    price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="下单时单价"
    )
    quantity = models.IntegerField(verbose_name="购买数量")
    total_price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="小计金额"
    )
